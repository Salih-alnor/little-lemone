...
const Home = ({ navigation }) => {
  ...
  const menuCategory = ["All", "Mains", "Desserts", "Starters", "Specials"];
  ...
  const filterItems = (query) => {
    ...
    const filteredData = fullData.filter(({ category, name }) => {
      const formattedCategory = category.toLowerCase().replace(/\s+/g, "");
      const formattedName = name.toLowerCase().replace(/\s+/g, "");
  
     
      return (
        formattedCategory.includes(formattedQuery) ||
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.heroSection}>
          <View>
            <Text style={{ fontSize: 35, color: "#F4CE14" }}>
              Little Lemone
            </Text>
          ...
          <View style={styles.search}>
            <Image
              style={{ width: 25, height: 25, tintColor: "#495E57" }}
              source={Search}
            />
            <TextInput
              ...
              autoCapitalize="none"
              placeholder="Search"
              placeholderTextColor={Platform.OS === "ios" ? "grey" : "grey"}
              autoCorrect={false}
            ...
            {menuCategory.map((item, index) => {
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.sectionBtn,
                    { marginRight: index === item.length - 1 ? 32 : 6 },
                  ]}
                  key={index}
                >
                  <Text style={{ fontWeight: "600", color: "#495E57" }}>
                    {item}
                  </Text>
                </TouchableOpacity>
              );
